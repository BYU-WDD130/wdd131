...
from number_entry import IntEntry  # si no lo usas, puedes eliminar esta línea
...
class BakeryApp:
    def __init__(self, master):
        ...
        self.tree = ttk.Treeview(master, columns=("ID","Name","Price","Stock","Qty"), show="headings", height=10)
        ...
        # Permitir editar la columna "Qty"
        self.tree.bind("<Double-1>", self.on_double_click)

        # Botón de generar orden
        self.order_button = tk.Button(master, text="Process Order", command=self.process_order, bg="#f4a3b4", font=("Helvetica", 12, "bold"))
        self.order_button.pack(pady=10)

        # Área de texto para recibo
        self.receipt_text = tk.Text(master, height=15, font=("Courier", 10))
    ...
    def on_double_click(self, event):
        ...
        if column == "#5":  # columna de Quantity
            x, y, width, height = self.tree.bbox(item, column)
            entry = tk.Entry(self.master)
            entry.place(x=x + 40, y=y + 120, width=60, height=25)
            entry.focus()
            ...
            def save_edit(event):
                ...
                try:
                    int(new_value)
                    self.tree.set(item, "Qty", new_value)
                except ValueError:
